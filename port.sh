...
# Seamless AOD: Setting_AodState lives in Settings.Secure (a database,
# not a boot property), so it must be set via the settings CLI AFTER
# SettingsProvider/system_server is fully up -- sys.boot_completed=1,
# not "on boot" (which fires too early for this to succeed).
cat > "${_dbg_sys}/etc/init/zzz-seamless-aod.rc" << 'AODRC'
on property:sys.boot_completed=1
    exec - system system -- /system/bin/settings put secure Setting_AodState 1
    exec - system system -- /system/bin/settings put secure Setting_AodEnableImmediate 1
AODRC
...
if [[ $port_android_version -ge 16 ]];then
    if [[ -f $targetSettings ]];then
        blue "WHICH FAVOR OF YOUR LORD WILL YOU DENY??"
        ...
        const-string v0, \"${port_oplusrom_version} \u209a\u2091\u0064\u2091\u209b\u209c\u1d63\u1d62\u2090\u2099\"
...
# --- kebab camera fix: newer OplusCamera.apk (common_cos, my_product)
# paired with an OnePlus9-sourced camera algorithm library set (odm) since
# no kona/SM8250-family camera6.0/5.0-fix_odm exists upstream. Byte-identical
# .so files confirmed across two unrelated newer-chipset donor packages
# (OnePlus9/SM8350 and OP4E3F/sm8750), suggesting these algo libraries are
# chipset-generic; oplus_camera_config JSON values patched for this
# device's own IMX481 ultra-wide sensor resolution. Runs before
# kebab.prop/build.prop edits, ordinary pipeline position. ---
# --- TEMP: camera-fix (COS part) disabled for isolated vl53l1-only test ---
if [[ "${base_product_device}" == "SKIP_OnePlus8T" ]]; then
    if [[ -f devices/common/camera6.0-fix_cos.zip ]]; then
        blue "kebab camera fix: applying common COS 6.0 camera app"
...
# --- end kebab camera fix (COS/app part only -- ODM part moved to run
# after gt_neo3t_audio_fix, see below, so the wholesale odm/lib replace
# there can't overwrite these camera algorithm libraries) ---

# --- vl53l1 disable: INDEPENDENT of the camera-fix on/off switch above.
# This unit has no ToF/laser sensor hardware, so vl53l1_daemon_main
# crash-loops trying to start, leaving the camera app with a null
# depth map -- suspected root cause of both the portrait-mode hang and
# (per the same over-amplified ToF-tuned white-balance matrix) the
# Night Sight purple/magenta tint. Disable it outright, always, for
# this device, regardless of which camera app/odm set is active. ---
if [[ "${base_product_device}" == "OnePlus8T" ]] && [[ -f build/portrom/images/vendor/etc/init/hw/init.qcom.rc ]]; then
    blue "kebab camera fix: disabling absent-hardware vl53l1_daemon_main"
    python3 devices/kebab/audio_hal_32bit_fix/disable_vl53l1.py \
        build/portrom/images/vendor/etc/init/hw/init.qcom.rc
fi
# --- end vl53l1 disable ---

# --- Bangladesh (mcc470) carrier_volte_available_bool fix: this ROM's
# CarrierConfig.apk is a stripped-down build covering only mcc204/310/311/460
# (NL/US/CN), so any unlisted carrier -- including all of Bangladesh --
# falls through to the framework's conservative default (VoLTE disabled).
# Added per-carrier mccmnc override XMLs for Grameenphone/Robi/Banglalink/
# Teletalk (47001-47004) inside a rebuilt CarrierConfig.apk. ---
if [[ -f devices/common/carrierconfig_patched.apk ]]; then
    blue "Applying Bangladesh carrier VoLTE config fix"
    cp devices/common/carrierconfig_patched.apk \
        build/portrom/images/system_ext/priv-app/CarrierConfig/CarrierConfig.apk
fi
...
# --- re-apply 32-bit camera calibration libs: gt_neo3t_audio_fix (above)
# wholesale-replaces vendor/lib with GT Neo 3T's own copy, which lacks
# these two 8T-native camera-calibration libraries added to the master
# list earlier in the pipeline. Copy them back in here, after the
# wholesale replace, so they survive onto the final image. ---
if [[ -f devices/kebab/audio_hal_32bit_fix/libVDDualCameraBlurlessAPI.so ]]; then
    cp -f devices/kebab/audio_hal_32bit_fix/libVDDualCameraBlurlessAPI.so \
        build/portrom/images/vendor/lib/libVDDualCameraBlurlessAPI.so
fi
if [[ -f devices/kebab/audio_hal_32bit_fix/libarcsoft_calibverify_TriCamera.so ]]; then
    cp -f devices/kebab/audio_hal_32bit_fix/libarcsoft_calibverify_TriCamera.so \
        build/portrom/images/vendor/lib/libarcsoft_calibverify_TriCamera.so
fi
chmod 0644 build/portrom/images/vendor/lib/libVDDualCameraBlurlessAPI.so \
    build/portrom/images/vendor/lib/libarcsoft_calibverify_TriCamera.so 2>/dev/null
# --- end re-apply 32-bit camera calibration libs ---

# --- kebab camera fix (ODM part): runs here, AFTER gt_neo3t_audio_fix's
# wholesale odm/lib replace, so these camera algorithm libraries are the
# final, un-overwritten state on the built image. ---
# --- TEMP: camera-fix (ODM part) disabled for isolated vl53l1-only test ---
if [[ "${base_product_device}" == "SKIP_OnePlus8T" ]] && [[ -f devices/OnePlus8T/camera5.0-fix_odm.zip ]]; then
    blue "kebab camera fix: applying device-specific ODM algorithm set"
    unzip -o devices/OnePlus8T/camera5.0-fix_odm.zip -d build/portrom/images/
fi
# --- end kebab camera fix (ODM part) ---
